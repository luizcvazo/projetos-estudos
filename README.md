### REPOSITÓRIO DE ESTUDOS E PRÁTICAS EM PROGRAMAÇÃO

Portfólio de evolução de técnica em Front‑End e Back‑End baseados em cursos livres e Analise e Desenvolvimento de Sistemas

------------------------------------------------------------------------

# Sobre o Repositório:
Este repositório reúne projetos desenvolvidos com foco em:

-   Consolidação de fundamentos de desenvolvimento web
-   Boas práticas de estruturação de código
-   Organização de projetos reais
-   Evolução contínua em front-end e back-end

Cada pasta representa um projeto de estudos ou conjunto de exercícios
aplicados.
Este ambiente funciona como um laboratório prático de aprendizado,
experimentação e melhoria contínua.

------------------------------------------------------------------------

# Competências de Estudos:

✔ Estruturação semântica com HTML5\
✔ Layouts modernos com CSS3 (Flexbox e Grid)\
✔ Manipulação de DOM com JavaScript\
✔ Lógica de programação aplicada\
✔ Organização modular de projetos\
✔ Versionamento com Git\
✔ Scripts e exercícios em Python

------------------------------------------------------------------------

## Projetos Presentes:

Projeto Portfolio - Landing page de apresentação profissional.

Projeto Tela de Login - Interface de autenticação com foco em layout e experiência do usuário.

Projeto To‑Do List - Aplicação interativa utilizando manipulação de DOM e lógica JavaScript.

Projeto Redes Sociais - Simulação de interface inspirada em redes sociais modernas.

Projeto Cordel - Projeto visual com foco em tipografia, responsividade e estruturação semântica.

Projeto Android - Estudo de layout e organização estrutural.

Estudos em Python - Exercícios voltados para lógica, estrutura condicional e fundamentos de programação.

Outros Projetos Personalizados - Projetos aplicados para prática de desenvolvimento real.

------------------------------------------------------------------------

## Tecnologias Utilizadas

-   HTML5
-   CSS3
-   JavaScript (ES6+)
-   Python
-   Git & GitHub

------------------------------------------------------------------------

## Como Executar

Clone o repositório:

``` bash
git clone https://github.com/luizcvazo/projetos-estudos.git
```

Acesse a pasta do projeto desejado e abra o arquivo HTML no navegador:

``` bash
cd projetos-estudos
cd nome-do-projeto
```

------------------------------------------------------------------------

## Objetivo Profissional:
Este repositório representa minha evolução técnica como desenvolvedor,
com foco em:

-   Desenvolvimento Front-End
-   Estruturação de aplicações web
-   Organização de código escalável
-   Preparação para integração com back-end e banco de dados

Estou constantemente expandindo os projetos aqui presentes e
implementando melhorias progressivas.

------------------------------------------------------------------------

## Contato:
Se você é recrutador ou profissional da área e deseja conversar:

GitHub: https://github.com/luizcvazo

------------------------------------------------------------------------

## Considerações Finais:
Este repositório demonstra:

-   Consistência de estudos
-   Aplicação prática de conceitos
-   Organização e clareza estrutural
-   Comprometimento com evolução profissional
-   Melhoria constante e aperfeiçoamento do conhecimento

Obrigado por visitar.
